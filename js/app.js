...
(function () {
  ...
  btnUseKnee.addEventListener('click', () => {
    ...
    // UX: si se usa estimación, se anula la talla manual
  ...
  function recalcAll() {
    ...
    if (formula) {
      ...
      const plan = window.FormulasUtils.calcEnteralPlan(req.actual, formula);
      if (!plan.ok) {
        ...
        setText('#covPct', fmtPct(plan.covPct));
    ...
    // Notas/advertencias
    const planForNotes = formula ? window.FormulasUtils.calcEnteralPlan(req.actual, formula) : null;
  ...
  function renderGlobalNotes({ formula, plan, reqActual }) {
    ...
    if (!formula) {
      ...
      if (plan && plan.ok) {
        ...
        if (Number.isFinite(plan.covPct)) {
          const diff = Math.abs(plan.covPct - 100);
          if (diff > 20) notes.push({ kind: 'danger', text: `Cobertura alejada del 100% (${plan.covPct.toFixed(0)}%): el redondeo/objetivo puede estar desajustado.`, });
          else if (diff > 10) notes.push({ kind: 'warn', text: `Cobertura no exacta por redondeo (${plan.covPct.toFixed(0)}%).`, });
        }
      }
  ...
  function renderSummary({ mst, tallaCm, imc, glim, req, formula }) {
    ...
    if (formula) {
      ...
      if (plan.ok) {
        ...
        lines.push(`Aporte real: ${Math.round(plan.kcalReal)} kcal/día (${fmtPct(plan.covPct)})`);
